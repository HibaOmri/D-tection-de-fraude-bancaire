...
def generate_shap_plots(model_path='models/best_model_sota.joblib', train_path='data/fraudTrain.csv', output_dir='reports/figures', n_samples=500):
    ...
    print("=== CHARGEMENT DU MODÈLE ET PRÉPARATION SHAP ===")
    ...
    # Échantillonnage équilibré pour la rapidité du calcul SHAP
    ...
    print(f"Calcul des SHAP values sur {len(X_sample)} transactions de validation...")
    explainer = shap.TreeExplainer(model)
    shap_values = explainer(X_sample)
    
    # Gestion du cas multi-classe / binaire
    if len(shap_values.shape) == 3: # (samples, features, classes)
        shap_vals_class1 = shap_values[:, :, 1]
    else:
        shap_vals_class1 = shap_values
        
    # 1. SHAP Summary Plot (Importance globale des variables)
    ...
    plt.close()
    print(f"SHAP Summary Plot enregistré : {summary_path}")
    
    # 2. SHAP Waterfall Plot pour une transaction frauduleuse spécifique (Explication locale)
    ...
    if len(fraud_idx_in_sample) > 0:
        ...
        plt.close()
        print(f"SHAP Waterfall Plot enregistré : {waterfall_path}")
